...
function jannah_extensions_shortcodes_scripts() {
	...
	$lang_text = array(
		 ...
		 // Not used anymore but after updating the plugin on some sites the cached version of the mce.js file still loaded instead of the new file which cause issues, will be removed in the future
		...
		'shortcode_sponsored'     => esc_html__( 'Sponsored?',   'jannah-extensions' ),
		...
		'shortcode_centered'      => esc_html__( 'Centered?',              'jannah-extensions' ),
		...
		'shortcode_flickr_id'     => esc_html__( 'Account ID  ( get it from https://idgettr.com )', 'jannah-extensions' ),
...
function jannah_extensions_sc_author_info( $atts, $content = null ) {
	...
	$avatar = apply_filters( 'tie/extensions/shortcodes/author/avatar', '<img src="'. esc_attr( $image ) .'" alt="'. $title .'" class="author-avatar-img" width="111" height="111" />' );
...
function jannah_extensions_sc_button( $atts, $content = null ) {
	...
	$rel = '';
	if( $nofollow == 'true' || $sponsored == 'true' ){
		$rel = ' rel="';
	...
	$out = '<a href="'. esc_url( $link ) .'"'. $target . $rel .' class="shortc-button '. $size .' '. $color .'">'. $icon . do_shortcode( $content ). '</a>';

	if( ! empty( $center ) && $center == 'true' ){
		return '<div class="tie-center-button">'. $out .'</div>';
	}
...
function jannah_extensions_sc_facebook( $atts, $content = null ) {
	$post_id = get_the_ID();
	return '<iframe loading="lazy" src="https://www.facebook.com/plugins/like.php?href='. get_permalink( $post_id ) .'&amp;layout=box_count&amp;show_faces=false&amp;width=100&amp;action=like&amp;font&amp;colorscheme=light&amp;height=65" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:50px; height:65px;" allowTransparency="true" async></iframe>';
}
...
function jannah_extensions_sc_tweet( $atts, $content = null ) {
	...
	return '<a href="'. esc_url( 'https://twitter.com/share' ) .'" class="twitter-share-button" data-url="'. get_permalink( $post_id ) .'" data-text="'. get_the_title( $post_id ) .'" data-via="'. $username .'" data-lang="en" data-count="vertical" >tweet</a><script async src="https://platform.twitter.com/widgets.js"></script>';
}
...
function jannah_extensions_sc_pinterest( $atts, $content = null ) {
	...
	return '
		...
		<a href="https://pinterest.com/pin/create/button/?url='. get_permalink( $post_id ) .'&amp;media='. get_the_post_thumbnail( $post_id, 'large' ) .'" class="pin-it-button" count-layout="vertical">
...
function jannah_extensions_sc_post_slideshow( $atts, $content = null ) {
	...
	if( function_exists( 'is_amp_endpoint' ) && is_amp_endpoint()  ){
		...
		return '<amp-carousel width="700" height="418" type="slides" layout="responsive" class="i-amphtml-layout-responsive i-amphtml-layout-size-defined i-amphtml-element i-amphtml-built i-amphtml-layout" i-amphtml-layout="responsive">'. do_shortcode( $content ) .'</amp-carousel>';
	}
	else{
		return "
