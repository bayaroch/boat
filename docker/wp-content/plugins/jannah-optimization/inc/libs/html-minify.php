...
/**
	...
	Edited by TieLabs to fix bugs with Woocommerce
	...
	protected function minifyHTML( $html ) {
		...
		foreach ( $matches as $token ){
				...
				if ( $tag === 'pre' || $tag === 'textarea' ) {
					...
					if ( $tag !== '' ) {
						if ( strpos( $tag, '/' ) === false ) {
							...
							$content = preg_replace( '/(\s+)(\w++(?<!action|alt|content|src|value)=(""|\'\'))/i', '$1', $content );
						}
			...
			if ( $relate && $this->shorten_urls ) {
				...
				// Removed "Action" to fix an issue with the Sabai Discuss plugin (Edit Question) 
				$content = preg_replace_callback( '/(data|href|src)=(?:"([^"]*)"|\'([^\']*)\')/i', array( &$this,'callback_HTML_URLs' ), $content );
			}
