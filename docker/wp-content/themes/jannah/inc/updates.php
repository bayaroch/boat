...
function tie_theme_update_db(){
	...
	# Debug
	//update_site_option( 'tie_ver_'. TIELABS_THEME_ID, '6.0.0' );
	...
	$current_version = get_site_option( 'tie_ver_'. TIELABS_THEME_ID ) ? get_site_option( 'tie_ver_'. TIELABS_THEME_ID ) : get_option( 'tie_ver_'. TIELABS_THEME_ID );
	...
	if( $current_version ){
		...
		if( version_compare( $current_version, TIELABS_DB_VERSION, '<' ) ){
			...
			if( version_compare( $current_version, '7.0.0', '<' ) ){
				...
				/*$options_to_update = array(
					...
					'tie_ver_'. TIELABS_THEME_ID,
					'tie_chnagelog_'. TIELABS_THEME_ID,
					'tie_token_error_'.TIELABS_THEME_ID,
					'tie_server_error_'.TIELABS_THEME_ID,
					TIELABS_THEME_SLUG .'_demos_count',
				);

				foreach ( $options_to_update as $single_option ) {
					if( get_option( $single_option ) ){
						update_site_option( $single_option, get_option( $single_option ) );
					}
					delete_transient( $single_option );
				}*/
        ...
        - Fix: Sticky video issue.
			...
			if( ! empty( $changelog ) ){
				...
				// Store the new data
				update_site_option( 'tie_chnagelog_'. TIELABS_THEME_ID, trim( $changelog ), false );
			...
			// ----
			delete_site_option( 'tie_token_error_'.TIELABS_THEME_ID );
			delete_site_transient( 'tie_server_error_'.TIELABS_THEME_ID );
