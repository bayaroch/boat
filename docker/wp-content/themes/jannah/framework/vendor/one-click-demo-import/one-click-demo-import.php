...
class PT_One_Click_Demo_Import {
	...
	public function create_plugin_page(){
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'Install Demos', TIELABS_TEXTDOMAIN ),
	...
	// by TieLabs ***************************************************************
		...
		if( $demos_count ){
			update_site_option( TIELABS_THEME_SLUG .'_demos_count', $demos_count, false );
		}
	...
	<div id="tie-import-wrap" class="ocdi wrap about-wrap tie-about-wrap">
		...
		<h2 class="tie-import-title"><?php esc_html_e( $plugin_page_setup['page_title'] ) ?></h2>
		...
		$extra_demos = tie_get_latest_theme_data( 'demos_extra' );

		$support_info = tie_get_support_period_info();

		if( tie_get_token() && ! empty( $extra_demos ) && ! empty( $support_info['status'] ) && $support_info['status'] == 'expired' ){
			?>
			<div class="tie-message-hint tie-message-success">
				<?php 
					printf(
						esc_html__( 'Your Support Period has expired on %1$s, You can import any of the following demos that have been released before the end of your support period.', TIELABS_TEXTDOMAIN ),
						'<strong>'. $support_info['human_date'] .'</strong>'
					);
				?>
			</div>
			<?php 
			...
			<?php foreach ( $this->import_files as $index => $import_file ) :	?>
				<div class="theme">
					<div class="theme-screenshot" data-value="<?php echo esc_attr( $index ); ?>">
						...
						<span class="more-details"><?php esc_html_e( 'Import', TIELABS_TEXTDOMAIN ); ?></span>
						<?php
					...
					<div class="theme-id-container">
						<h3 class="theme-name"><?php esc_html_e( $import_file['import_file_name'] ); ?></h3>
						<?php
							if( ! empty( $import_file['import_demo'] ) ) { ?>
								<div class="theme-actions">
									<a class="tie-live-demo button button-secondry" target="_blank" href="<?php echo esc_url( $import_file['import_demo'] ); ?>"><?php esc_html_e( 'Live Demo', TIELABS_TEXTDOMAIN ); ?></a>
								</div>
								<?php
							}
						?>
		...
		if ( ! empty ( $extra_demos ) ) :

		?>
		<div class="" style="background: #fffae1; padding: 4% 4% 1%; overflow:hidden; border: 5px solid #fbdf51; margin-top: 50px;">
				
			<h3 style="margin-top: 0"><?php esc_html_e( 'More Demos', TIELABS_TEXTDOMAIN ) ?></h3>
		<?php
			if( ! empty( $support_info['status'] ) && $support_info['status'] == 'expired' ){
			?>
				<div class="tie-message-hint tie-message-error">
					<?php 
						printf(
							esc_html__( 'Your Support Period has expired on %1$s, All of the following demos have been released after the end of your support period. you need to renew your support period or get the VIP Access to be able to import these demos.', TIELABS_TEXTDOMAIN ),
							'<strong>'. $support_info['human_date'] .'</strong>'
						);
					?>
				</div>
		...
		?>
			...
			<div class="theme-browser tie-demo-importer rendered" id="ocdi__demo-import-files">
				<?php foreach ( $extra_demos as $single_extra ) :	?>
					<div class="theme">
						<div class="theme-screenshot extra-demos">
							<div class="demo-desc"><?php echo ! empty( $single_extra['desc'] ) ? wp_kses_post( $Parsedown->text( $single_extra['desc'] ) ) : ''; ?></div>
							<img src="<?php echo esc_attr( $single_extra['img'] ); ?>">
							<span class="more-details"><?php echo esc_html__( 'More Details', TIELABS_TEXTDOMAIN ); ?></span>
						</div>

						<div class="theme-id-container">
							<h3 class="theme-name"><?php esc_html_e( $single_extra['name'] ); ?></h3>

							<div class="theme-actions">
								<a class="tie-live-demo button button-secondry" target="_blank" href="<?php echo esc_url( $single_extra['url'] ); ?>"><?php esc_html_e( 'Live Demo', TIELABS_TEXTDOMAIN ); ?></a>
							</div>
					...
					<div class="theme-about wp-clearfix">
						...
						<?php
							...
							if( ! tie_get_token() ){
								...
								?>
							<div id="import-demos-notices" class="tie-message-hint">
								<h4>
	...
	public function import_demo_data_ajax_callback(){
		...
		$memory = 'ini'.'_'.'set'; $memory( 'memory_limit', apply_filters( 'pt-ocdi/import_memory_limit', '950M' ) ); //##### add @ later
		...
		if ( ! $use_existing_importer_data ){
			...
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ){ // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				// Custom Thumb sizes // TieLabs
				if( ! empty( $this->import_files[ $this->selected_index ]['import_file_name'] ) ) {

					if( $this->import_files[ $this->selected_index ]['import_file_name'] == 'Creative' ){

						$updated_options = get_option( apply_filters( 'TieLabs/theme_options', '' ) );

						$updated_options[ 'image_size_'. TIELABS_THEME_SLUG .'-image-small'] = array(
							'width'  => 150,
							'height' => 150,
							'crop'   => true,
						);

						$updated_options[ 'image_size_'. TIELABS_THEME_SLUG .'-image-large'] = array(
							'width'  => 390,
							'height' => 390,
							'crop'   => true,
						);
						...
						/* 
						$updated_options[ 'image_size_'. TIELABS_THEME_SLUG .'-image-post'] = array(
							'width'  => 780,
							'height' => 470,
							'crop'   => true,
						);
	...
	private function import_content( $import_file_path ){
		...
		@set_time_limit( apply_filters( 'pt-ocdi/set_time_limit_for_demo_data_import', 900 ) );
