	...
	class TIELABS_NOTICES {
		...
		function load_notices(){
			...
			if ( $theme_pages == 'toplevel_page_tie-theme-options' ){ 
				...
				// Need Help pointer
				if( ! self::is_dismissed( 'tie_help_icon_'. TIELABS_THEME_SLUG ) ){
				...
				add_action( 'admin_notices', array( $this, 'rate_theme_version' ), 115 );
		...
		function need_help_script(){
			...
			$pointer_content .= '<p style="font-size: 14px;">'. sprintf( esc_html__( 'Check the documentation from here, or click on the help icon %s that appears beside each setting if you need help.', TIELABS_TEXTDOMAIN ), '<span class="dashicons dashicons-editor-help"></span>' ) .'</p>'; ?>

			<script>
				//<![CDATA[
				jQuery(document).ready( function($){
					$('#tiepanel-head-elements-help').pointer({
						...
						close: function(){
							$.post( ajaxurl, {
								pointer: 'tie_help_icon_<?php echo esc_js( TIELABS_THEME_SLUG ) ?>',
		 ...
		 * Rate V5
		 */
		function rate_theme_version(){

			$notice_id = 'tie_bigupdate_date_7_'. TIELABS_THEME_ID;
			...
			if( ! get_option( $notice_id ) ){
				...
				if( ( time() - get_option( $notice_id ) ) < DAY_IN_SECONDS ){
					return false;
				}

				$notice_content  = '<h4 style="color: #42067e; font-size: 26px;">'. esc_html__( 'Biggest Update Ever', TIELABS_TEXTDOMAIN ) .'</h4>';
				$notice_content .= '<p style="font-size: 15px; margin-bottom: 10px;">'. esc_html__( 'We want to provide you with a great experience which is why we want to hear from you. Your feedback helps us bring you more of the features you love and the improvements you expect.', TIELABS_TEXTDOMAIN ) .'</p>';
				$notice_content .= '<p style="font-size: 16px; margin-bottom: 10px; font-weight: bold;">'. esc_html__( 'We hope you love the new features and improvements, We\'d be thrilled if you could rate Jannah 5 stars &#x2b50;&#x2b50;&#x2b50;&#x2b50;&#x2b50; on ThemeForest, that will help us a lot on improving our items and the support as well.', TIELABS_TEXTDOMAIN ) .'</p>';
				$notice_content .= '<p style="font-size: 15px; margin-bottom: 25px;">'. esc_html__( 'Thank you in advance for your collaboration. We really appreciate your time!', TIELABS_TEXTDOMAIN ) .'</p>';
				$notice_content .= '<a href="'. tie_get_purchase_link( array( 'utm_medium' => 'rate-v7' ) ) .'" target="_blank" style="background-color: #42067e; border:none; font-size: 20px; width: 100%; text-align:center;" class="button button-primary button-hero">'. esc_html__( 'Leave a Review on ThemeForest', TIELABS_TEXTDOMAIN ) .'</a>';

				echo '<div id="tie-page-overlay" class="is-notice-dismissible" data-id="'. $notice_id .'" style="bottom: 0; opacity: 0.8;"></div>';

				self::message( array(
					'notice_id' => $notice_id,
					'title'     => '<img style="max-width: 300px;" src="'. TIELABS_TEMPLATE_URL .'/framework/admin/assets/images/jannah-7.png" />',
					'message'   => $notice_content,
					'class'     => 'sucess tie-popup-block tie-popup-window tie-notice-popup tie-yay',
				));
			}
		...
		function rate_theme(){
			...
			if( empty( $install_date ) || $install_date > 1670227911 ){ // If the install date is after 05 v6 release date
		...
		function theme_translation(){
			...
			if( ! tie_get_latest_theme_data( 'translations_url' ) ){
				return false;
			}
			...
			self::message( array(
				...
				'button_url'  => tie_get_latest_theme_data( 'translations_url' ),
			));
		}
		...
		function new_update(){
			...
			if ( ! self::is_dismissed( $notice_id ) && $changelog = get_site_option( 'tie_chnagelog_'. TIELABS_THEME_ID ) ){
		...
		function new_milestone(){
			...
			if( tie_get_option( 'tie_post_views' ) == 'theme' && ! $milestone ){
				...
				global $wpdb;
				$number = $wpdb->get_var( $wpdb->prepare( " SELECT sum(meta_value) FROM $wpdb->postmeta WHERE meta_key = %s", TIELABS_HELPER::get_views_meta_field() ) );
		...
		function happy_anniversary(){
			...
			if( ! empty( $customer_since ) ) {
				...
				if( $current_month == $customer_month && $customer_year < $current_year ){
					...
					$notice_content = '<span class="tie-number-of-years">'. $number_of_years .'</span>';
			...
			<div id="<?php echo esc_attr( sanitize_key( $args['notice_id'] ) ) ?>" class="tie-notice <?php echo esc_attr( $args['class'] ); ?>" style="<?php echo esc_attr( $args['color'] ); ?>">
				...
				<div class="tie-notice-content">
					...
					if( ! empty( $args['button_text'] ) ){ ?>
						<a class="tie-primary-button button button-primary button-hero <?php echo esc_attr( $args['button_class'] ) ?>" href="<?php echo esc_url( $args['button_url'] ) ?>"><?php esc_html_e( $args['button_text'] ) ?></a>
						<?php
					}
					?>
					...
					if( ! empty( $args['button_2_text'] ) ){ ?>
						<a class="tie-primary-button button button-primary button-hero <?php echo esc_attr( $args['button_2_class'] ) ?>" href="<?php echo esc_url( $args['button_2_url'] ) ?>"><?php esc_html_e( $args['button_2_text'] ) ?></a>
						<?php
					}
					?>
