...
if( ! class_exists( 'TIELABS_DEMO_IMPORTER' ) ) {
	...
	class TIELABS_DEMO_IMPORTER{
		...
		function __construct(){
			...
			add_action( 'TieLabs/Demos/after_page_title',  array( $this, 'after_page_title' ));
		...
		public function is_rtl(){
			...
			// Since 4.7.0 users can set the preferred language check get_user_locale
			...
			$rtl_langs = array( 'ur', 'snd', 'he', 'fa-af', 'ar', 'fa_IR' );

			if( in_array( get_locale(), $rtl_langs ) ){				
				return true;
			}
		}
		 ...
		 * after_page_title
		 */
		public function after_page_title(){
		 ...
		 */
		 public function demo_page_setup( $settings ){
			...
			if( $demos_count && get_site_option( TIELABS_THEME_SLUG .'_demos_count' ) < $demos_count ){
				$new_demos    = $demos_count - get_site_option( TIELABS_THEME_SLUG .'_demos_count' );
			...
			$settings['page_title']  = esc_html__( 'Install Demos', TIELABS_TEXTDOMAIN );
		...
		public function before_import(){
			...
			// --
			if( function_exists( 'get_user_locale' ) && ( get_user_locale() != get_locale() ) && function_exists( 'wp_get_current_user' ) ){
				$user = wp_get_current_user();
				update_user_meta( $user->ID, 'tie_saved_locale', $user->locale );
				update_user_meta( $user->ID, 'locale', get_locale() );
			}
		...
		public function import_settings( $file ){
			...
			if( $this->is_rtl() && ! empty( $settings['post_content_rtl'] ) ){
				$post_content = $settings['post_content_rtl'];
			}
			else{
				$post_content = $settings['post_content'];
			}

			$categories_ids = $this->insert_posts( $thumbnails_ids, $post_content );
			...
			if( ! empty( $homepage_id ) ){
				...
				foreach( $sections as $section_index => $section_data ){
					...
					// Section Sidebar
					if( $this->is_rtl() && ! empty( $section_data['settings']['sidebar_position'] ) && $section_data['settings']['sidebar_position'] != 'full' ){
		...
		public function after_import(){
			...
			$user = wp_get_current_user();
			$saved_locale = get_user_meta( $user->ID, 'tie_saved_locale', true );
			
			if( ! empty( $saved_locale ) ){
				update_user_meta( $user->ID, 'locale', $saved_locale );
				delete_user_meta( $user->ID, 'tie_saved_locale' );
			}
		}
		...
		private function insert_single_main_menu( $categories_ids ){
		  ...
		  $menu_icon = $this->is_rtl() ? 'fa-align-left' : 'fa-align-right';
