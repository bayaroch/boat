...
class TIELABS_FILTERS {
	...
	function __construct() {
		...
		// Google Search
		add_filter( 'get_search_query',  array( $this, 'google_search_query' ) );
		add_filter( 'search_template_hierarchy',  array( $this, 'google_search_template' ) );
		
		// Display The Notification Bar
		add_filter( 'TieLabs/before_wrapper',  array( $this, 'notification_bar' ) );
		
		// Validate the license please
		add_filter( 'TieLabs/before_wrapper',  array( $this, 'theme_validation_message' ) );
		...
		// Comments
		add_filter('comment_form_default_fields', array( $this, 'comments_disable_website' ) );

		// Quote default style
		if( tie_get_option( 'quote_style' ) ){
			add_filter( 'the_content', array( $this, 'quote_default_style' ) );
		}
	...
	function meta_description(){
		...
		// Return if the description option is disabled or the Yoast SEO, SEOPress or Rank Math SEO plugins are active
		if( ! tie_get_option( 'post_meta_escription' ) || class_exists( 'WPSEO_Frontend' ) || class_exists( 'RankMath' ) || function_exists( 'seopress_init' ) ){
			return;
		}
	...
	function footer_misc(){
		...
		if ( tie_get_option( 'reading_indicator' ) && is_single() && ! tie_is_loaded_posts_active() ){

			if( tie_get_option( 'reading_indicator_pos' ) == 'top' && ! tie_get_option( 'stick_nav' ) ){
				echo '<div id="reading-position-indicator" class="reading-indicator-top"></div>';
			}
			else{
			...
			if( ! empty( $current_category->term_id ) ){
				...
				if( ! empty( $posts_order ) ){
					...
					if( $posts_order == 'views' && tie_get_option( 'tie_post_views' ) ){ // Most Viewd posts
						...
						$query->set( 'meta_key', TIELABS_HELPER::get_views_meta_field() );
					}
				...
				// ---
				$asc_or_desc = tie_get_category_option( 'asc_or_desc', $current_category->term_id );
				if( ! empty( $asc_or_desc ) ){
					$query->set( 'order', $asc_or_desc );
				}
	...
	function redirect_random_post(){
		...
		if( isset( $_GET['random-post'] ) && (
			( tie_get_option('top_nav') && tie_get_option('top-nav-components_random') ) ||
			( tie_get_option( 'main_nav' ) && tie_get_option( 'main-nav-components_random' ) )
			) ) {

			$args = apply_filters( 'TieLabs/redirect_random_post/args', array(
				'posts_per_page'      => 1,
				'orderby'             => 'rand',
				'fields'              => 'ids',
				'no_found_rows'       => true,
	 ...
	 */
	function has_post_thumbnail( $has_thumbnail, $post, $thumbnail_id ) {

		if( get_post_type( $post ) == 'post' && tie_get_option( 'default_featured_image' ) && tie_get_option( 'default_featured_image_id' ) ){
			$has_thumbnail = true;
		}
	...
	/**
	 * Set a default quote style
	 */
	function quote_default_style( $content ) {

		if( ( function_exists( 'is_amp_endpoint' ) && is_amp_endpoint() ) || is_admin() ){
			return $content;
		}

		$class = 'quote-' . tie_get_option( 'quote_style' );
		$content = str_replace( 'quote-simple', $class, $content );
		$content = str_replace( 'quote-light', $class, $content );
		$content = str_replace( '<blockquote>', '<blockquote class="'.$class.'">' , $content );
		$content = str_replace( 'wp-block-quote', 'wp-block-quote '.$class , $content );
	...
	/**
	 * Change the default ?s with ?q in the search query
	 */
	function google_search_query( $query = '' ){

		if( tie_get_option( 'google_search_engine_id' ) ){
			if( is_search() && ! empty( $_GET['q'] ) ){
				return esc_attr( wp_strip_all_tags( $_GET['q'] ) );
			}
		}
	...
	/**
	 * Redirect the default search template into the custom Google Search template
	 */
	function google_search_template( $templates ){

		if( tie_get_option( 'google_search_engine_id' ) ){
			return array( 'templates/google-search.php' );
		}
	...
	function notification_bar(){
		...
		if( ! tie_get_option( 'notification_bar' ) || tie_is_auto_loaded_post() ){
			return;
		}
		?>

		<div id="header-notification-bar">
			<div class="container" <?php if ( tie_get_option( 'notification_bar_centered' ) ) echo 'style="justify-content: center;"'; ?>>
				<p><?php echo tie_get_option( 'notification_bar_content' ); ?></p>
				<?php
					if( tie_get_option( 'notification_bar_button_url' ) && $text = tie_get_option( 'notification_bar_button_text' ) ){
						$target   = tie_get_option( 'notification_bar_button_tab' ) ? 'target="_blank"' : '';
						$nofollow = tie_get_option( 'notification_bar_button_nofollow' ) ? 'rel="nofollow noopener"' : '';
						echo '<a class="button" href="'. esc_url( tie_get_option( 'notification_bar_button_url' ) ) .'" title="'. esc_attr( $text ).'" '. $target .' '. $nofollow .'>'. $text .'</a>';
					}
				?>
			</div><!-- .container -->
	 ...
	 */
	function theme_validation_message(){
		
		if( is_user_logged_in() && current_user_can( 'manage_options' ) && ! tie_get_token() && ! tie_is_auto_loaded_post() && ! empty( $_SERVER['SERVER_NAME'] ) && strpos( $_SERVER['SERVER_NAME'], 'tielabs.com' ) === false && $_SERVER['SERVER_NAME'] != 'localhost'  ){
			?>
	 ...
	 */
	function comments_disable_website( $fields ){

		if( tie_get_option( 'comments_disable_url' ) &&  ! empty( $fields['url'] ) ){
			unset( $fields['url'] );
		}
