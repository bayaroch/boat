...
if( ! class_exists( 'TIELABS_STYLES' ) ) {
	...
	class TIELABS_STYLES {
		...
		public static function init(){
			...
			// Add Colors to Color Palette in Gutenberg Block Editor
			add_action( 'after_setup_theme',         array( __CLASS__, 'block_editor_palette' ) );

			// Head codes
			add_action( 'wp_head',                   array( __CLASS__, 'insert_external_fonts_head' ), 20 );
			add_action( 'wp_head',                   array( __CLASS__, 'custom_head_code' ) );
			add_action( 'wp_head',                   array( __CLASS__, 'theme_color_meta_tag' ) );
			add_action( 'wp_head',                   array( __CLASS__, 'meta_viewport' ), 5 );
		 ...
		 * Add Colors to Color Palette in Gutenberg Block Editor
		 */
		public static function block_editor_palette() {
			...
			if ( isset( $settings['color']['palette']['default'] ) && is_array( $settings['color']['palette']['default'] ) ) {
		...
		public static function localize_script(){
			...
			// Check if the side aside has a light skin, Dark is the default
			$side_aside_light = false;
			if( tie_get_option( 'side_aside_skin') == 'light' ){
			...
			$js_vars = array(
				...
				'is_side_aside_light'    => $side_aside_light,
				...
				'autoload_posts'         => ( is_single() && tie_is_loaded_posts_active() ),
				...
				'sticky_share_post_menu' => ( tie_get_option( 'single_sticky_menu' ) && tie_get_option( 'share_post_sticky_menu' ) ),
			);

			if( tie_get_option( 'ad_blocker_detector' ) && tie_get_option( 'ad_blocker_detector_delay' ) ){
				$js_vars['ad_blocker_detector_delay'] = (int) tie_get_option( 'ad_blocker_detector_delay' ) * 1000; // convert to ms
			}

			if( tie_get_option( 'ad_blocker_disallow_images' ) ){
				$js_vars['ad_blocker_disallow_images_placeholder'] = tie_get_option( 'ad_blocker_disallow_images_placeholder', TIELABS_TEMPLATE_URL.'/assets/images/message-placeholder.png' );
				$js_vars['ad_blocker_disallow_images_post'] = tie_get_option( 'ad_blocker_disallow_images_post' );
			}
	  ...
	  */
		...
		public static function is_inline_css(){
			...
			return apply_filters( 'TieLabs/Styles/is_inline_css', true );
		...
		public static function custom_theme_color( $color = false ){
			...
			$color = ! empty( $color ) ? $color : tie_primary_color( false );
		...
		public static function builder_block_style( $block ){
			...
			if( ! empty( $block['boxid'] ) ){
				...
				// Sliders
				if( strpos( $block['style'], 'slider_' ) !== false  && ! empty( $block['sec_color'] ) ){
					$out .= "
						$id_css .thumb-title,
						$id_css .thumb-title a,
						$id_css .thumb-desc,
						$id_css .thumb-meta{
							color: {$block['sec_color']} !important
						}
					";
				}
		 ...
		 */
		public static function theme_color_meta_tag(){

			$theme_color = false;

			$theme_color_type = tie_get_option( 'theme_color_meta_tag_type' );

			// Theme Primary Color
			if( $theme_color_type == 'primary' ){
				$theme_color = tie_primary_color();
			}

			// Header Color
			elseif( $theme_color_type == 'header' ){
				if( tie_get_option( 'header_layout' ) == 1 || tie_get_option( 'header_layout' ) == 4 ){
					$theme_color = tie_get_option( 'main_nav_background' );
				}
				else{
					$theme_color = tie_get_option( 'header_background_color' );
				}
			}

			// Custom Color
			elseif( $theme_color_type == 'custom' ){
				$theme_color = tie_get_option( 'theme_color_meta_tag_custom' );
			}

			$theme_color = apply_filters( 'TieLabs/theme_color_meta_tag_color', $theme_color );
