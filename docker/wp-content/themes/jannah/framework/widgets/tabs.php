	 ...
	 */
	 class TIE_WIDGET_TABS extends WP_Widget {
		...
		public function widget( $args, $instance ){
			...
			<div id="<?php echo esc_attr( $widget_id ) ?>" class="container-wrapper tabs-container-wrapper tabs-container-<?php echo count( $tabs_order_array ) ?>">
				...
				<select id="<?php echo esc_attr( $this->get_field_id( 'posts_order' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'posts_order' ) ); ?>" class="widefat tie-posts-order-option">
					...
					<?php 
						if( tie_get_option( 'tie_post_views' ) == 'theme' ){ ?>
							...
							<option value="views_7_days" <?php selected( $posts_order, 'views_7_days' )  ?>><?php esc_html_e( 'Most Viewed for 7 days', TIELABS_TEXTDOMAIN) ?></option>					
						<?php 
						}
					?>
				</select>
